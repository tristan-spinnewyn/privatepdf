...
export const ImagesToPdfTool: React.FC = () => {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      fireSuccessConfetti();
      setSuccessMessage(t('tools.imagesToPdf.success', { count: items.length }));
    } catch (err) {
      console.error(err);
      setErrorMessage("Error converting images / Erreur de conversion.");
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-900 tracking-tight">{t('tools.imagesToPdf.headerTitle')}</h2>
          <p className="text-sm text-slate-500 mt-0.5">
            {t('tools.imagesToPdf.headerDesc')}
          </p>
        </div>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white p-5 rounded-3xl border border-slate-200/80 shadow-xs grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1.5">
                {t('tools.imagesToPdf.orientation')}
              </label>
              <select
                value={orientation}
              ...
              >
                <option value="auto">{t('tools.imagesToPdf.orientationAuto')}</option>
                <option value="portrait">{t('tools.imagesToPdf.orientationPortrait')}</option>
                <option value="landscape">{t('tools.imagesToPdf.orientationLandscape')}</option>
              </select>
            </div>

            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1.5">
                {t('tools.imagesToPdf.margins')}
              </label>
              <select
                value={margin}
                ...
                className="w-full px-3 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-hidden focus:ring-2 focus:ring-rose-500/20"
              >
                <option value="normal">{t('tools.imagesToPdf.marginNormal')}</option>
                <option value="small">{t('tools.imagesToPdf.marginSmall')}</option>
                <option value="none">{t('tools.imagesToPdf.marginNone')}</option>
              </select>
            </div>

            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1.5">
                {t('tools.imagesToPdf.pdfName')}
              </label>
              <input
                type="text"
            ...
            {items.map((item, index) => (
              ...
              >
                ...
                <div className="px-3 py-2 bg-slate-50/90 border-b border-slate-100 flex items-center justify-between text-xs">
                  <span className="font-bold text-slate-700">{t('tools.organize.page')} {index + 1}</span>
          ...
          <div className="sticky bottom-6 bg-white/95 backdrop-blur-md p-4 sm:p-5 rounded-3xl border border-slate-200/90 shadow-xl flex items-center justify-between gap-4 z-20">
            <div className="text-xs sm:text-sm text-slate-600 font-medium">
              {t('tools.imagesToPdf.summary', { count: items.length })}
            </div>
            ...
            >
              {isProcessing ? (
                <>
                  ...
                  <span>{t('tools.imagesToPdf.processing')}</span>
                </>
              ) : (
                <>
                  <Download className="w-5 h-5" />
                  <span>{t('tools.imagesToPdf.btnGenerate')}</span>
                </>
              )}
            </button>
          </div>
